...
<html lang="en">
  ...
  <body>
	...
	<div class='container' style='margin-top: 20px'>
		<form action="?go=ler" method="post">
		<h3>
		<span class="label label-default">ID</span>
		<input type="text" name="id" id="id" maxlength="60"/>
		</h3>
		</br></br>
		<h4>
	...
	<?php
		if(@$_GET['go']=='ler'){
			...
			$query = "SELECT * FROM pedido WHERE id_pedido LIKE '%".$id."%'";
			...
			<tbody>
			<?php
				while($linha = mysqli_fetch_assoc($ok))
				{
					echo'<tr>';
						echo'<td>'.$linha["id_pedido"].'</td>';
						echo'<td>'.$linha["id_prod_ped"].'</td>';
						echo'<td>'.$linha["id_cliente_ped"].'</td>';
					echo'</tr>';
				}
